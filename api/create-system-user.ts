...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const body: CreateSystemUserBody = typeof req.body === 'string' ? JSON.parse(req.body || '{}') : (req.body ?? {})
  ...
  if (!SYSTEM_ROLES.includes(body.role as typeof SYSTEM_ROLES[number])) {
    return res.status(400).json({ error: 'Invalid role for this endpoint — work roles are created on Staff Management.' })
  }
